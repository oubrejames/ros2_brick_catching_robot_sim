...
class Catcher(Node):
    ...
    def __init__(self):
        ...
        self.sub_brick_status = self.create_subscription(
            Bool, "brick_status", self.brick_status_callback, 10)
        self.sub_reset = self.create_subscription(
            Bool, "reset_sim", self.reset_callback, 10)
        self.pub_send_turtle_robot = self.create_publisher(
            Bool, "send_turtle_robot", 10)
        self.pub_is_brick_caught = self.create_publisher(
            Bool, "brick_caught", 10)
        self.turtle_init = PoseStamped()
        self.turtle_init_flag = True
        self.brick_status = False
        self.counter = 0
        self.pub_text = self.create_publisher(
            Marker, "text_marker", 10)  # Marker publisher for text
        self.text_counter = 0
        self.flag = False
    ...
    def detect_falling(self):
        ...
        distance_to_brick = np.sqrt((self.brick_pose.x -
                                     self.turtle_pose.x)**2 +
                                    (self.brick_pose.y -
                                     self.turtle_pose.y)**2)
        # Can robot make it to goal in time?
        if (self.max_velocity *
                time_to_platform) < distance_to_brick and self.state == State.FALLING:
