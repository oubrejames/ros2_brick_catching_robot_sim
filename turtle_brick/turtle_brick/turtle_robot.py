...
class TurtleRobot(Node):
    ...
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter("wheel_radius", 0.2, ParameterDescriptor(
            description="The radius of the turtle robot wheel in meters"))
        ...
        self.pub_tilt_to_arena = self.create_publisher(
            Bool, "tilt_in_arena", 10)
        self.odom_publisher = self.create_publisher(Odometry, "odom", 10)

        # Subscribers
        self.sub_turtle_pose = self.create_subscription(
            Pose, "turtle1/pose", self.listener_callback_turtle_pose, 10)
        self.sub_goal_pose = self.create_subscription(
            PoseStamped, "goal_pose", self.listener_callback_goal_pose, 10)
        self.tilt_sub = self.create_subscription(
            Tilt, "tilt", self.tilt_callback, 10)
        self.sub_reset = self.create_subscription(
            Bool, "reset_sim", self.reset_callback, 10)
        self.is_brick_caught = self.create_subscription(
            Bool, "brick_caught", self.brick_caught_callback, 10)
        self.sub_go_robot = self.create_subscription(
            Bool, "send_turtle_robot", self.go_robot_callback, 10)
    ...
    def cmd_vel_to_goal(self):
        ...
        if abs(x) < 0.01 and abs(y) < 0.01:
            ...
            if (self.goal_pose.pose.position.x -
                self.turtle_init.x) < 0.01 and (self.goal_pose.pose.position.y -
                                                self.turtle_init.y) < 0.01:
    ...
    def timer_callback(self):
        ...
        self.joints.position = [
            float(
                self.stem_turn_rads), float(
                self.wheel_turn_rads), float(0.0), float(
                self.platform_tilt_rads)]
        self.joints.velocity = [
            float(
                self.stem_turn_vel), float(
                self.wheel_turn_vel), float(0.0), float(
                self.platform_tilt_vel)]
        self.pub_joints.publish(self.joints)
        ...
        quat_base = quaternion_from_euler(
            float(0), float(0), float(
                self.turtle_pose.theta))  # Roll pitch yaw
